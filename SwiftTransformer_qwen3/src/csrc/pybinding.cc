...
#include "model/gpt/qwen3/qwen3op.h"  // 要实现的 Qwen3Op 类头文件
...
TORCH_LIBRARY(gpt_ops, m) {
  ...
  m.class_<st::model::Qwen3Op>("Qwen3Op")
    ...
    .def("forward", &forwardWrapper<st::model::Qwen3Op>)
    .def("init_communicator", &initCommunicatorWrapper<st::model::Qwen3Op>)
  ;
}
