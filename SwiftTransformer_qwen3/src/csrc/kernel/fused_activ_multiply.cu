...
//SwiGLU激活函数调用
template<typename T>
__global__ void fusedSwiGLUMultiplyKernel(
	T* output,
	const T* input1,
	const T* input2,
	int64_t n
) {
	using T2 = std::conditional_t<std::is_same_v<T, half>, half2, float2>;
	for (int64_t i = blockIdx.x * blockDim.x + threadIdx.x; i < n/2; i += blockDim.x * gridDim.x) {
		T2 a = ((const T2*)input1)[i];
		T2 b = ((const T2*)input2)[i];
		T2 r = {
			applySwiGLU<T>(a.x, b.x),
			applySwiGLU<T>(a.y, b.y)
		};
		((T2*)output)[i] = r;
	}
}
...
void fusedActivationMultiply(
	...
	switch (activation_type) {
		...
		case ActivationType::GELU:
			...
			return;
		case ActivationType::kSwiGLU:
		    fusedSwiGLUMultiplyKernel<T><<<gridSize, blockSize>>>(output, input1, input2, n);
		    return;
		default:
			assert (false);
	}
